...
{ 
  ...
  if (this.level>0) 
    if (this.isLastNode) //the last child in the children array 
	    leftSide = leftSide + "<td valign=middle>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=7 height=7 border=0></a></td>"
    else 
      leftSide = leftSide + "<td valign=middle background=" + ICONPATH + "ftv2blank.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=7 height=7 border=0></a></td>"
  ...
  {
	  if (this.prependHTML == "")
        docW = docW + "<img src=" + ICONPATH + "ftv2blank.gif height=20 width=2>"
  }
  if (WRAPTEXT)
...
{ 
  ...
  if (isOpen == this.isOpen) 
    return 
  //alert("setStateFolder");
  if (browserVersion == 2)  
  { 
    //alert("browserVersion == 2");
    totalHeight = 0 
    for (i=0; i < this.nChildren; i++) 
  ...
  {
    // alert("this.getID()!=foldersTree.getID() && PRESERVESTATE && !this.isOpen");
  ...
  if (!this.isOpen && this.isLastOpenedfolder)
  {
		...
		//alert("!this.isOpen && this.isLastOpenedfolder");
  }
...
{ 
  ...
  { 
    ...
    if (browserVersion > 0) 
      docW = docW + "onClick='javascript:return clickOnFolder(\""+this.getID()+"\")'"
...
function nodeImageSrc() {
  ...
  { 
    if (this.nChildren == 0)
      srcStr = ICONPATH + "arrow_right.jpg"
    else
      if (this.isOpen)
        srcStr = ICONPATH + "arrow_down.jpg"  
      else
        srcStr = ICONPATH + "arrow_right.jpg"  
  } 
  else 
...
{ 
  ...
  if (this.level>0) 
    ...
    { 
      leftSide = leftSide + "<td valign=middle><img src='" + ICONPATH + "arrow_right.jpg' width=4 height=7></td>"
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=middle background=" + ICONPATH + "ftv2blank.gif><img src='" + ICONPATH + "arrow_right.jpg' width=16 height=0></td>"
    } 
  ...
  if (USEICONS)
	  ...
	  if (this.prependHTML == "")
        docW = docW + "<img src=" + ICONPATH + "ftv2blank.gif height=22 width=3>"
...
{
  //alert("forceOpeningOfAncestorFolders");
...
{ 
  ...
  { 
    //alert("escondeBlock one or three"+browserVersion);
    if (this.navObj.style.display == "none") 
      return 
    this.navObj.style.display = "none" 
  } else {
  //alert("escondeBlock one or three else part"+browserVersion);
...
function blockStartHTML(idprefix) {
  ...
  if (browserVersion == 2) 
    docW = "<layer "+ idParam + " middle=" + doc.yPos + " visibility=show>"
  else if (browserVersion != 0)
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top background=" + ICONPATH + "ftv2blank.gif><img src='" + ICONPATH + "ftv2blank.gif' width=16 height=22></td>"
		}
...
{ 
   ...
   var vinval3 = 0
    ...
    var url = getElById('itemTextLink'+clicked.id);
    var url1=url.toString();    
    if(checkURL(url))
    {
      var cond=confirm("Unsaved Data may be lost, do you want to continue ?");
      if(cond)
      {
         return true;
      }
    ...
    if (clicked.nChildren==0) {
      ...
      clicked.isLastOpenedfolder = false
    }
...
{
   //currentOpen = GetCookie("clickedFolder")
   splitval=cookieCutter
    vinval = currentOpen.split(splitval)
    vinval2 = vinval[0]
    vinval3 = vinval[1]

   //alert("currentopen is -->"+currentOpen )
   //alert("vinval2 is -->"+vinval2 )
   //alert("vinval3 is -->"+vinval3 )
   vinval4 = clicked.level 
 }    

   if (currentOpen != null && vinval3 != 0 && vinval4 ==1 )
   {   
    splitval=cookieCutter
    vinval = currentOpen.split(splitval)
    vinval2 = vinval[0]
    prefolderObj = findObj(vinval2)
    preopenobj = prefolderObj.id   
      //preopenobj.isOpen = false
      clickOnNode(preopenobj); //sets lastOpenedFolder to null



   }

//changes made by vinod ends here
...
if (currentOpen != null )
{
   ...
   //alert("clickOnNode is -->")
  // alert("vinval4 is -->"+vinval4 )
   //alert("vinval3 is -->"+vinval3 )
 }  
   if (currentOpen != null && vinval3 != 0 && vinval4 ==1)
   {
...
function highlightObjLink(nodeObj) 
{
  ...
  //session.putValue("checkURL",checkURL);  
...
function preLoadIcons() {
	...
	auxImg.src = ICONPATH + "arrow_down.jpg";
	auxImg.src = ICONPATH + "arrow_down.jpg";
	auxImg.src = ICONPATH + "arrow_right.jpg";
	auxImg.src = ICONPATH + "arrow_right.jpg";
	auxImg.src = ICONPATH + "ftv2blank.gif";
...
{
	url1 = url.toString();	
	var strarr=url1.split('/Portal/'); 	
	var strarr1 =strarr[1].split("?"); 	
	return (strarr1[0]=='requestNewPasswordAC.do') ;
}
